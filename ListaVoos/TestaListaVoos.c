...
void testa_busca_voo_nulo(void) { 
    ...
    char *p_destino = (char *)malloc(sizeof(char)*300); 
    leitura_voo(p_voo3,&codigo,p_origem,p_destino);
    print_teste(busca_voo_codigo(NULL,10) == NULL && busca_voo_codigo(p_lista,codigo + 1) == NULL, "busca_voo() - teste 1: vôo nulo");
    libera_lista(p_lista); 
    free(p_origem); free(p_destino); 
}
void testa_busca_voo_valido(void) { 
    Voo *p_voo = cria_voo("Fortaleza","Natal");
    Voo *p_voo2 = cria_voo("Natal","Maceió");
    Voo *p_voo3 = cria_voo("Fortaleza","Porto Alegre");
    ListaVoo *p_lista = cria_lista(); 
    insere_voo(p_voo,p_lista); 
    insere_voo(p_voo2,p_lista); 
    insere_voo(p_voo3,p_lista);
    ...
    leitura_voo(p_voo2,&codigo2,p_origem,p_destino); 
    leitura_voo(p_voo,&codigo,p_origem,p_destino);
    print_teste(busca_voo_codigo(p_lista,codigo3) == p_voo3 && busca_voo_codigo(p_lista,codigo) == p_voo && busca_voo_codigo(p_lista,codigo2) == p_voo2, "busca_voo() - teste 2: vôo válido");
    libera_lista(p_lista); 
    free(p_origem); free(p_destino); 
}

void testa_cria_roteiro_voo() {
    printf("Testando cria_roteiro_voo()...\n");
    Voo *p_voo1 = cria_voo("Fortaleza","Natal");
    Voo *p_voo2 = cria_voo("Natal","Maceió");
    Voo *p_voo3 = cria_voo("Fortaleza","Porto Alegre");
    Voo *p_voo4 = cria_voo("Maceió","Curitiba");

    ListaVoo *p_listaVoo = cria_lista();
    insere_voo(p_voo1, p_listaVoo);
    insere_voo(p_voo2, p_listaVoo);
    insere_voo(p_voo3, p_listaVoo);
    insere_voo(p_voo4, p_listaVoo);

    ListaVoo *p_resultado = cria_roteiro_voo(p_listaVoo, "Fortaleza", "Porto Alegre");

    print_lista_voo(p_resultado);
    ListaVoo *p_resultado2 = cria_roteiro_voo(p_listaVoo, "Fortaleza", "Curitiba");
    inverte_lista_voo(p_resultado2);
    print_lista_voo(p_resultado2);
}
