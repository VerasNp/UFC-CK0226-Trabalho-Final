...
/*Acessa os atributos do passageiro*/
...
int passageiro_libera(Passageiro *p_passageiro){
    if (p_passageiro != NULL){
        if (p_passageiro->endereco == NULL || p_passageiro->nome == NULL) printf("NULO\n");
...
int get_passageiro_codigo(Passageiro *p_passageiro) {
    return p_passageiro->id;
}
